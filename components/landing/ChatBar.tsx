  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-[640px] mx-auto">
      ...
      <input
        ...
        aria-hidden="true"
      />
      <div className="flex items-stretch overflow-hidden bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl focus-within:border-[var(--color-accent)] transition-colors">
        <input
          id="chat"
          type="text"
          value={value}
          ...
          className="flex-1 min-w-0 bg-transparent px-5 py-4 outline-none font-voice text-[var(--color-text)] placeholder:text-[var(--color-text-muted)]"
        />
        <button
          type="submit"
          disabled={!value.trim() || status === "sending"}
          className="px-4 text-[var(--color-accent)] font-voice disabled:opacity-40"
        >
          →
        </button>
      </div>
